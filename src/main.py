...
# Habilita CORS para permitir chamadas do Chainlink Functions
CORS(app)
...
@app.route('/api/analyze-sentiment-chainlink', methods=['POST'])
def analyze_sentiment_chainlink():
    """
    Endpoint específico para Chainlink Functions
    Analisa uma imagem IPFS e retorna análise de sentimentos estruturada
    """
    print("Recebendo requisição do Chainlink Functions para análise de sentimentos")
    ...
    try:
        ...
        image_hash = data['image_hash']
        print(f"Analisando imagem para Chainlink Functions: {image_hash}")
        ...
        # Usa a mesma função de análise existente
        image_url = f"https://gateway.pinata.cloud/ipfs/{image_hash}"
        analysis = analyze_image_with_openai(image_url)
        
        # Estrutura a resposta para o Chainlink Functions
        chainlink_response = {
            "sentiment_analysis": analysis.get("sentiment_analysis", ""),
            "color_psychology": analysis.get("color_psychology", ""),
            "symbol_relation": analysis.get("symbol_relation", ""),
            "visual_language": analysis.get("visual_language", ""),
            "keywords": ",".join(analysis.get("keywords", [])),
            "attributes": json.dumps(analysis.get("attributes", []))
        }
        
        print("Análise concluída com sucesso para Chainlink Functions")
        return jsonify(chainlink_response), 200
        
    except Exception as e:
        print(f"Erro ao analisar imagem para Chainlink Functions: {str(e)}")
        return jsonify({'error': str(e)}), 500
